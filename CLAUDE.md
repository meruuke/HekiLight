...
An **OnUpdate polling loop** (not purely event-driven) because the SBA suggestion can change every frame:

- `StartPollLoop()` — sets `OnUpdate` on `display`; fires `Refresh()` every `db.pollRate` seconds (default 0.05 s)
- `StopPollLoop()` — clears the script on combat-end or when SBA is inactive
...
2. **Fallback**: derive spellID via `GetActionInfo` on the SBA slot from `C_ActionBar.FindAssistedCombatActionButtons()` / `IsAssistedCombatAction()`
...
- **Keybind lookup**: `spellID → FindSpellActionButtons() → filter SBA slots → SLOT_BINDINGS[slot] → GetBindingKey() → FormatKey()`
